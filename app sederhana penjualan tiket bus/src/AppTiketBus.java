...
public class AppTiketBus {
    ...
    private void writeToFile(String data) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("data_tiket.txt", true))) {
            if (data != null) {
                writer.write(data);
                writer.newLine();
            } else {
                System.err.println("Data is null. Unable to write to file.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void cetakTiket() {
        ...
        // Menampilkan data di GUI
        data.setText(ticketData);

        // Menuliskan data ke file
        writeToFile(ticketData);
    }
